...
const doc: ComponentContent = {
  ...
  <VisuallyHidden>Remove Climbing bean ‘Blue Lake’ seeds</VisuallyHidden>
    ...
    {
      title: "Finishing a repeated name",
      description:
...
<Input id="q" type="search" placeholder="Search…" />`,
      render: () => (
        <div style={{ display: "grid", gap: "var(--loam-space-2xs)", inlineSize: "100%" }}>
          <VisuallyHidden render={<label htmlFor="vh-demo-q" />}>
            Search the catalogue
          </VisuallyHidden>
          <input id="vh-demo-q" type="search" placeholder="Search…" />
        </div>
      ),
    },
  ],
  ...
  howItWorks: [
    {
      title: "Clipped, not removed",
      body: "The element is one pixel, clipped with clip-path and pulled back by a negative margin, so it takes no visible room but is still rendered. display: none and visibility: hidden would take it out of the accessibility tree along with the eye, which is the opposite of what this is for. white-space: nowrap keeps a long name from being broken one character per line before it is clipped.",
    },
    {
      title: "A span, until it is not",
      body: "The default element is a <span>, which is phrasing content and so legal inside a button, a label or a heading. Where the slot needs a different element — a <legend> for a fieldset, a <label> for a control — render swaps it and the class comes along.",
    },
    {
      title: "Compose the appropriate element",
      body: "Use VisuallyHidden for text that should remain available to assistive technology. Its render prop lets you preserve a label, legend or heading when a span is not appropriate. The loam-VisuallyHidden class remains available for existing integrations; use the component in new React compositions.",
    },
  ],
  accessibility: [
  ...
  props: [
    {
      name: "children",
      type: "ReactNode",
      description: "The text read by assistive technology.",
    },
    {
      name: "render",
      type: "RenderProp",
      description:
        "Render as a different element, e.g. render={<legend />}. The class and attributes merge onto it.",
    },
    {
      name: "...others",
