...
Use the component reference for exceptions. Badge, Loader, Progress and Meter have intrinsic `size` APIs; Input supports the native HTML `size` attribute, with adornments composed as siblings. Button icons are children. These are different contracts, not interchangeable conventions.
