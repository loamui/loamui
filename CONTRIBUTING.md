...
- `packages/create-loamui`: the `create-loamui` scaffolder and `doctor` setup
  checker, published independently of core. Its `assets/` are copies of the
  skill's Stylelint and composition-checker files, regenerated by
  `pnpm --filter create-loamui sync-assets` and gated by `check:cli-assets`.
...
- `pnpm check:cli-assets` fails if the setup assets bundled in
  `packages/create-loamui/assets` drift from `skills/loamui/assets`.
  `pnpm --filter create-loamui test` covers the scaffolder's detection and
  wiring logic without network access.
...
The public packages are `@loamui/core` and `create-loamui`; the monorepo root
